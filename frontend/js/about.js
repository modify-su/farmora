...
(function () {
  ...
  function resolveImgPath(src) {
    if (!src) return '../images/picture 1.jpg';
    if (src.startsWith('data:') || src.startsWith('http://') || src.startsWith('https://')) return src;

    const isRoot = !window.location.pathname.includes('/frontend/html');
    if (isRoot) {
      // หน้า root เช่น /about.html
      return src.replace(/^\.\.\/frontend\/images\//, 'images/')
                .replace(/^\.\.\/images\//, 'images/')
                .replace(/^images\//, 'images/');
    } else {
      // หน้าใน /frontend/html/about.html
      return src.replace(/^\.\.\/frontend\/images\//, '../images/')
                .replace(/^images\//, '../images/')
                .replace(/^\.\.\/images\//, '../images/');
    }
