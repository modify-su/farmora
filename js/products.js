...
(function () {
  ...
  function resolveImgPath(src) {
    ...
    if (!src) {
      return isRoot ? 'frontend/images/product-fertilizer.jpg' : '../images/product-fertilizer.jpg';
    }
      ...
      return `
        <article class="prod-card" data-id="${p.id}" data-cat="${p.cat}">
          <div class="pc-img-wrap" onclick="window.FarmoraProducts.openDetail('${p.id}')">
            <img src="${resolveImgPath(p.image)}" alt="${p.name}" loading="lazy" onerror="this.src='${resolveImgPath('product-fertilizer.jpg')}'">
  ...
  function openDetail(id) {
    ...
    if (pdmMainImg) pdmMainImg.src = resolveImgPath(item.image);
